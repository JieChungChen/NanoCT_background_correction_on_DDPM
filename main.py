...
def get_args_parser():
    ...
    parser.add_argument('--checkpoint', default='dref_256_base.pt', type=str)   
    # Training Settings
...
def main(args):
    ...
    trainer = GaussianDiffusionTrainer(model, args.beta_1, args.beta_T, args.beta_sche, args.T, args.uncon_ratio).to(device)
    model.train()
    ...
    for e in range(args.epoch):
        ...
        if e % 5 == 0:
            dataset = NanoCT_Pair_Dataset(data_dir=args.data_dir, img_size=args.img_size)
            if is_distributed:
