...
def get_args_parser():
    ...
    parser.add_argument('--configs', default='configs/ddpm_pair_finetune.yml', type=str)
    return parser
...
def main(args):
    ...
    dataset = NanoCT_Pair_Dataset(data_configs)
    if is_distributed:
        train_sampler = DistributedSampler(dataset, shuffle=True)
        dataloader = DataLoader(dataset, batch_size=trn_configs['batch_size'], num_workers=8, drop_last=True, pin_memory=True, sampler=train_sampler)
    else:
        dataloader = DataLoader(dataset, batch_size=trn_configs['batch_size'], num_workers=8, drop_last=True, pin_memory=True, shuffle=True)
